...
class CriticNetwork(nn.Module):
    def __init__(self, beta, input_dims, fc1_dims, fc2_dims, n_actions, name, checkpoint_dir = 'tmp\ddpg'):
        ...
        # Definition of first fully connected (FC) layer
        self.fc1 = nn.Linear(*self.input_dims, self.fc1_dims) # No idea what's happening here
        ...
        # Definition of second fully connected (FC) layer
    ...
    def forward(self, state, action):
        ...
        state_value = self.bn1(state_value) # First ReLU or first batch norm? Phil says first use the batch norm first
...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        # Bind output between -1 and 1 (tanh activation function)
        x = T.tanh(self.mu(x))
...
class Agent(object):
    ...
    def choose_action(self, observation):
        self.actor.eval() # Check video at 33:30 for explanation on this method
        observation = T.tensor(observation, dtype=T.float).to(self.actor.device)
        mu = self.actor(observation, dtype = T.float).to(self.actor.device)
    ...
    def learn(self):
        ...
        target_actions = self.target_action.forward(new_state)
